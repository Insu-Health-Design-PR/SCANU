...
# Allow importing layer4_inference when this script is run directly.
...
def _extract_thermal_model_scores(
    ...
    """
    Run Layer4 model over the capture video and align outputs to capture-frame indices.

    Returns:
        (thermal_model_scores, thermal_model_labels, thermal_model_confidences, sampled_count, warning)
    """
    n = len(frames)
    scores = [0.0] * n
    labels = ["unarmed"] * n
    ...
    for i in sampled_idxs:
        # Map capture index to video index linearly.
        if n <= 1:
            v_idx = 0
        else:
        ...
        try:
            ...
            # Keep defaults for this frame on inference failures.
            continue

    cap.release()

    # Forward-fill sampled predictions so each capture frame has a value.
    last_score = 0.0
    last_label = "unarmed"
    last_conf = 0.0
    for i in range(n):
...
def main() -> int:
    ...
    for i, f in enumerate(frames):
        ...
        # Fused score with thermal model contribution.
