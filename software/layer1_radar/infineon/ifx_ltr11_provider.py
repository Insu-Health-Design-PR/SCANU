"""Infineon BGT60LTR11AIP provider via `ifxradarsdk`.

This uses Infineon's Radar SDK Python wrapper (built/installed separately) to read
LTR11 frames and map the device metadata into the project's `PresenceProvider`
contract.
"""
...
class IfxLtr11PresenceProvider:
    ...
    def __init__(self, uuid: str | None = None, config: IfxLtr11ProviderConfig | None = None) -> None:
        # Import lazily so the rest of the repo can run without the SDK installed.
        ...
        # Device must be configured before acquisition can start.
    ...
    def close(self) -> None:
        try:
            ...
            self._started = False
            try:
                self._dev._close()  # type: ignore[attr-defined]
            except Exception:
                pass
    ...
    def read_sample(self) -> tuple[float, float, float]:
        ...
        Mapping:
        - presence_raw: motion metric computed from frame-to-frame delta energy
        - motion_raw: `1.0` if motion flag set else `0.0`
        - distance_m: not available from LTR11 metadata -> `-1.0`
        """
        ...
        # Power-like metric (for reference only).
        mag = np.abs(arr)
        signal_rms = float(np.sqrt(np.mean(mag * mag))) if mag.size else 0.0

        # Motion-like metric: RMS of delta between consecutive frames.
        motion_energy = 0.0
