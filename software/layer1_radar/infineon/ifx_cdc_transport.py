"""
Low-level transport for Infineon IFX CDC devices (e.g. DEMO BGT60LTR11AIPTOBO1).

This device enumerates as a CDC ACM serial port (e.g. /dev/ttyACM0) and speaks a
binary framed protocol. The full command set is vendor-specific; this module
implements:
- opening the port
- CRC16-CCITT-FALSE used by the device framing
- sending raw frames and reading replies

This is enough to *capture and inspect* traffic on Jetson even before the full
SDK/protocol is integrated.
"""
...
class IfxCdcTransport:
    ...
    def request(self, frame: bytes, read_bytes: int = 4096) -> bytes:
        """Send raw bytes and read whatever reply is available."""
    ...
    def request_cmd4(self, cmd: int, a: int = 0, b: int = 0, c: int = 0) -> IfxCdcReply:
        """
        Send a 4-byte command frame + CRC16 and read a 6-byte reply if present.

        This matches the observed behavior of the device when any byte is sent.
        """

        payload = bytes([cmd & 0xFF, a & 0xFF, b & 0xFF, c & 0xFF])
        crc = crc16_ccitt_false(payload)
        frame = payload + bytes([(crc >> 8) & 0xFF, crc & 0xFF])
        raw = self.request(frame, read_bytes=64)
        if len(raw) < 6:
