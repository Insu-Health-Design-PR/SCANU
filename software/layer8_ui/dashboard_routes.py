...
from layer4_inference.weapon_ai.live_frame_ipc import LiveBgrFrameReader, LiveFrameReader
...
def _patch_aiortc_h264_encoder_for_jetson() -> str:
    """
...
def _unique_profile_key_from_name(name: str, existing: set[str]) -> str:
    """Stable slug for JSON object key (no separate user-chosen id)."""
...
def build_router(layer8_dir: Path) -> APIRouter:
    ...
    webcam_ipc_reader = LiveFrameReader(Path("/dev/shm/scanu_webcam_live_frame.bin"))
    webcam_ipc_bgr_reader = LiveBgrFrameReader(Path("/dev/shm/scanu_webcam_live_bgr_frame.bin"))
    router = APIRouter()
    ...
    def snapshot_model_profile(body: SnapshotModelProfileBody) -> dict[str, Any]:
        ...
        if not pid:
            ...
            pid = _unique_profile_key_from_name(name_in, existing)
    ...
    def preview_command(sensor: SensorName) -> dict[str, Any]:
        ...
        run_cwd = str(sensor_runner.command_cwd(sensor, s))
        return {
            "command": cmd,
            "cwd": run_cwd,
        ...
        try:
            while True:
                ...
                if runner_on:
                    ...
                    rpath = resolved_artifact_path(s, relative_to_software=str(rel), layer8_dir=layer8_dir)
                    jpg: bytes | None = webcam_ipc_reader.read_latest()
                    if not jpg and rpath is not None and rpath.is_file():
                        try:
                            raw = await asyncio.to_thread(rpath.read_bytes)
                            if raw:
                                jpg = raw
    ...
    async def webrtc_webcam_offer(body: WebRTCOfferBody) -> dict[str, str]:
        ...
        class WebcamJpegTrack(VideoStreamTrack):
            ...
            async def recv(self) -> Any:
                ...
                self._next_t = max(self._next_t + (1.0 / self._fps), time.time())
                ...
                if bgr_payload is not None:
                    ...
                    if channels == 3 and len(raw) == expected:
                        ...
                        self._last_bgr = arr.reshape((int(height), int(width), 3)).copy()
                jpg = None
                ...
                if jpg and jpg != self._last_jpg:
                    ...
                    if decoded is not None:
                        self._last_bgr = decoded
                        self._last_jpg = jpg
                if self._last_bgr is None:
                    self._last_bgr = np.zeros((720, 1280, 3), dtype=np.uint8)
                bgr = self._last_bgr
                frame = VideoFrame.from_ndarray(bgr, format="bgr24")
                frame.pts = self._pts
                frame.time_base = Fraction(1, self._clock_hz)
                self._pts += int(self._clock_hz / self._fps)
                return frame
    ...
    async def stream_status() -> StreamingResponse:
        async def event_stream():
            try:
                while True:
                    payload = {
                        name: sensor_runner.status(name, layer8_dir) for name in ("thermal", "webcam", "mmwave")
                    }
