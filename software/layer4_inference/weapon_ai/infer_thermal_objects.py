...
from .live_frame_ipc import LiveBgrFrameWriter, LiveFrameWriter
...
def _add_bool_optional_arg(
    ...
    *,
    ...
    Python <3.9 lacks argparse.BooleanOptionalAction.
    """
...
def main() -> None:
    ...
    _add_bool_optional_arg(
        ...
        "--gun_take_best",
        default=True,
        help_text="Infer firearm YOLO at low conf and draw one best plausible box per person (default: on).",
    )
    p.add_argument(
        "--gun_thermal",
        action="store_true",
    ...
    is_linux_video_node = src_raw.startswith("/dev/video")
    src = int(src_raw) if src_raw.isdigit() else src_raw
    if isinstance(src, int) or is_linux_video_node:
        cap = None
    ...
    def _infer_annotations(
        ...
        if gun_detector is not None:
            ...
            if args.gun_full_frame:
                ...
                for ridx, (px1, py1, px2, py2, _prob, pcid, _ptag) in enumerate(rows):
        ...
        with torch.no_grad():
            while True:
                ...
                if fps_ema is not None:
                    ...
                    (tw, th), _ = cv2.getTextSize(fps_txt, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)
                    tx = max(8, w - tw - 12)
                    ty = max(th + 8, 24)
                    cv2.putText(
                        vis,
                ...
                if live_ipc_bgr_writer is not None:
                    hh, ww = vis.shape[:2]
                    live_ipc_bgr_writer.write(
                        vis.tobytes(),
                        height=int(hh),
                        width=int(ww),
                        channels=int(vis.shape[2] if vis.ndim >= 3 else 1),
                    )
